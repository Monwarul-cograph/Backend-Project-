...
from api.models import CEONumberID
import os
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    if "UnKnown" in str(row.get('代表電話番号')): 
                        d = ""
                   

                    if CEONumberID.objects.filter(代表電話番号=d):
                        duplicate = True

                    if not duplicate:
                        # Import company data with exception handling for each row
                        CEONumberID.objects.create(
                           代表電話番号 = row.get('代表電話番号', 'unknown')
                            )
            ...
            self.stdout.write(self.style.SUCCESS('Successfully imported CEONumberID'))
